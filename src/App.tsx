  ...
  const handleExport = async () => {
    ...
    try {
      ...
      for (const clip of clips) {
        ...
        args.push('-c:v', 'libvpx-vp9', '-b:v', '0', '-crf', '32', '-pix_fmt', 'yuv420p', '-c:a', 'libopus', '-b:a', '128k', outputName);
      ...
      if (needsFilter) {
        ...
        if (needsVideoEncode) {
          exportArgs.push('-c:v', 'libvpx-vp9', '-b:v', '0', '-crf', '28');
        } else {
          exportArgs.push('-c:v', 'copy');
        }
        if (needsAudioEncode) {
          exportArgs.push('-c:a', 'libopus');
        } else {
